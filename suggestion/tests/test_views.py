...
class SuggestionTestViews(TestCase):
    ...
    def test_edit_views(self):
        self.client.force_login(self.user)
        url = reverse('suggestion:edit', kwargs={'primary_key': self.suggestion.pk})
        response = self.client.get(url)
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'suggestion/form.html')

        data = {
            'book_title': 'This is the new book title',
            'comment': 'This is the new book comment', 
            'created_by': self.suggestion.created_by,
        }
        response = self.client.post(url, data)
        print("\nTest Data Used (Edit Suggestion):", data, "\n")
